...
func TestKeyID_AutoGeneratedOnSave(t *testing.T) {
	dir := t.TempDir()
	// Config has api_key but no key_id — SaveTo should derive and persist it.
	cfg := &Config{
		Context:   "test",
		Namespace: "default",
		APIKey:    "s3cr3t-token",
	...
	out := filepath.Join(dir, "kubeport.yaml")
	if err := cfg.SaveTo(out, FormatYAML); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
	// key_id should now be set in memory.
	if cfg.KeyID == "" {
		t.Fatal("expected key_id to be auto-populated after SaveTo")
	}
	...
	if reloaded.KeyID == "" {
		t.Fatal("expected key_id to be present in reloaded config")
	}
	if reloaded.KeyID != cfg.KeyID {
		t.Errorf("reloaded key_id = %q, want %q", reloaded.KeyID, cfg.KeyID)
	}
}

func TestKeyID_NotOverwrittenOnSave(t *testing.T) {
	dir := t.TempDir()
	// Config already has a user-supplied key_id — SaveTo must not overwrite it.
	cfg := &Config{
		Context:  "test",
		APIKey:   "s3cr3t-token",
		KeyID:    "my-custom-id",
		Services: []ServiceConfig{{Name: "api", Service: "svc", LocalPort: 8080, RemotePort: 80}},
	}
	out := filepath.Join(dir, "kubeport.yaml")
	if err := cfg.SaveTo(out, FormatYAML); err != nil {
		t.Fatalf("SaveTo: %v", err)
	}
	if cfg.KeyID != "my-custom-id" {
		t.Errorf("key_id was overwritten: got %q", cfg.KeyID)
	}
	reloaded, err := Load(out)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if reloaded.KeyID != "my-custom-id" {
		t.Errorf("reloaded key_id = %q, want %q", reloaded.KeyID, "my-custom-id")
	}
}
