...
func (a *app) cmdHelp() {
	...
	fmt.Println("                              Legacy: name:svc/target:remoteport:localport[:namespace]")
	fmt.Println("                              Multi:  name:svc/target:all[:+offset[:namespace]]")
	fmt.Println("                                      name:svc/target:http,grpc[:+offset[:namespace]]")
	fmt.Println("  --disable-svc <name>        Disable a service from config (repeatable)")
	fmt.Println("  --host <host:port>          Connect to a remote daemon (TCP mode)")
	fmt.Println("  --api-key <key>             API key for remote daemon authentication")
	fmt.Println("  --time <duration>           Refresh interval for watch command (default: 2s)")
	...
	fmt.Println("  --verbose                   Enable debug logging (overrides config log_level)")
	...
	fmt.Println("  kubeport start --svc \"api:svc/my-api:all\"         # Forward all ports")
	fmt.Println("  kubeport start --svc \"api:svc/my-api:http,grpc\"   # Forward named ports")
	fmt.Println("  kubeport start --svc \"api:svc/my-api:all:+10000\"  # All ports with offset")
	...
	fmt.Println("  KUBEPORT_API_KEY  API key for remote daemon authentication")
	fmt.Println()
